...
function generateCancellationICS(
  bookingId: string,
  title: string,
  description: string,
  location: string,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { booking_id, cancellation_reason, send_ics = true }: RequestBody = await req.json()
    ...
    const content = `
      ...
      ${cancellation_reason ? `
        <div style="background-color: ${EMAIL_COLORS.infoSection}; padding: 20px; border-radius: 6px; margin: 20px 0; border: 1px solid ${EMAIL_COLORS.border};">
          ...
          <p style="margin: 0; font-family: Arial, Helvetica, sans-serif;">${cancellation_reason}</p>
        </div>
      ` : ''}
...
${cancellation_reason ? `Cancellation Reason: ${cancellation_reason}` : ''}
...
Conference Hub Team
    ...
    // Add ICS cancellation attachment if requested
    if (send_ics) {
      const icsContent = generateCancellationICS(
        booking_id,
        booking.title,
        booking.description || 'Meeting cancellation',
