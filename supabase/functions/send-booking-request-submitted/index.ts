...
Conference Hub Team
    ...
    await transporter.sendMail({
      ...
      to: user.email,
      subject: `Booking Request Submitted: ${booking.title}`,
      html,
      text,
    })

    console.log(`✅ Booking request submitted email sent to ${user.email}`)

    return new Response(
      JSON.stringify({
        success: true,
        message: 'Booking request submitted email sent successfully',
    ...
    console.error('Error in send-booking-request-submitted function:', error)
    return new Response(
      JSON.stringify({
