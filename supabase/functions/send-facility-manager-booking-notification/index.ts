...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!manager || !manager.email || !manager.name) {
      return new Response(
        JSON.stringify({ error: 'Facility manager information not found' }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    const html = wrapEmailContent(content, 'New Booking Request', `Conference Hub - ${facility?.name || 'Facility Management'}`)
...
Conference Hub Team
    ...
    await transporter.sendMail({
      ...
      to: manager.email,
      subject: `New Booking Request: ${booking.title} - ${room?.name || 'Unknown Room'}`,
      html,
      text,
    })

    console.log(`✅ Facility manager notification email sent to ${manager.email}`)

    return new Response(
      JSON.stringify({
        success: true,
        message: 'Facility manager notification email sent successfully',
    ...
    console.error('Error in send-facility-manager-booking-notification function:', error)
    return new Response(
      JSON.stringify({
